<script>
    ...
    let errorMessage;
    fileAmountStore.subscribe((value) => fileAmount = value);
...
<div class="m-5">
    {#if progressInformationMessage && !errorMessage}
        <div class="flex items-center p-4 mb-4 text-sm text-blue-800 border border-blue-300 rounded-lg bg-blue-50 w-fit max-w-3xl" role="alert">
            ...
            <div>
                <span class="font-medium">Info!</span> {progressInformationMessage}
            </div>
        </div>
    {/if}

    {#if errorMessage}
        <div class="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 w-fit max-w-3xl" role="alert">
            <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
            </svg>
            <div>
                <span class="font-medium">An exception occurred!</span> {errorMessage}
            </div>
        </div>
    {/if}
    ...
    {#if diffDataStruct}
        <div class="flex">
            <div class="flex flex-col mr-2">
                ...
                {#if fileAmount && fileAmount !== diffDataStruct.length}
